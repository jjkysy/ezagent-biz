defmodule EzagentPluginMindmap.PersistenceTest do
  @moduledoc """
  增量 2 验收：Mindmap Kind 的节点树跨进程重启**durable 存活**。
  ...
  test "节点树跨重启存活（snapshot rehydrate）", %{uri: uri} do
    {:ok, pid1} = Ezagent.Kind.Server.start_link({Mindmap, %{uri: uri}})
    ...
    {:ok, _pid2} = Ezagent.Kind.Server.start_link({Mindmap, %{uri: uri}})
    :ok = wait_until_ready(uri)
