defmodule EzagentPluginMindmap.CapbacE2ETest do
  @moduledoc """
  增量3 DoD：per-node 授权经**真实 dispatch** 生效——非 owner 非 admin（但持 mindmap
  实例 cap）改一个别人认领的节点 → 被 CapBAC 拒；owner 本人改 → 通过。
  """
  use ExUnit.Case, async: false

  alias Ezagent.Invocation
  alias EzagentPluginMindmap.Mindmap
  ...
  test "非 owner 非 admin 被拒；owner 可改", %{uri: uri} do
    ...
    # 非通配的 mindmap 实例 cap（kind=:mindmap, action=:any）：能动 mindmap、但不是 admin。
    mm = Ezagent.Capability.cap(:mindmap, Ezagent.Behavior.Mindmap, :any)
    ...
    # alice 认领 n2（dispatch 授权过：她持 mindmap cap；handler：未分配可领）
    assert {:ok, %{}} = dispatch(uri, "claim_node", %{id: "n2"}, alice)

    # bob 持 mindmap cap（实例级授权过）但非 owner 非 admin → handler 的 per-node 拒
