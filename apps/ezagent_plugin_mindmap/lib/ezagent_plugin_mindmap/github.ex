defmodule EzagentPluginMindmap.Github do
  @moduledoc """
  GitHub REST 客户端（片6 出站）—— 把 mindmap 节点出站成 GitHub issue + post CI 评价评论。

  对称 `EzagentPluginMindmap.Miro`：`:httpc` + `Jason`，凭证从 `system://credentials/github.yaml`
  ...
  defp headers(token) do
    [
      ...
      {~c"User-Agent", ~c"ezagent-mindmap"},
