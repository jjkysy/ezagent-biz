defmodule EzagentPluginMindmap.Miro do
  @moduledoc """
  ...
  - 读节点：`GET /v2-experimental/boards/{board}/mindmap_nodes` → `%{"data" => [...]}`
  ...
  def create_node(token, board_id, content, parent_miro_id \\ nil) do
    ...
    case post(token, "/v2-experimental/boards/#{board_id}/mindmap_nodes", body) do
      {:ok, %{"id" => id}} -> {:ok, id}
      {:ok, other} -> {:error, {:unexpected_response, other}}
      {:error, _} = err -> err
    end
  ...
  def get_nodes(token, board_id) do
    case get(token, "/v2-experimental/boards/#{board_id}/mindmap_nodes") do
