defmodule Ezagent.World.MindmapDataTest do
  @moduledoc "验 world mindmap 面的读路径：read_tree 产出 JSON-safe 富节点树 + list_instances。"
  ...
  test "read_tree 返 JSON-safe 富树（stage/status 是 string、owner 带上、列表字段在）", %{uri: uri, ctx: ctx} do
    tree = MindmapData.read_tree(uri, ctx)
  ...
  test "list_instances 含刚 spawn 的实例", %{uri: uri} do
    uris = MindmapData.list_instances() |> Enum.map(& &1["uri"])
