# 02 · 战略 → 产品 → 开发 → 运营 → ROI 再循环（围绕"开发 ezagent 自己的产品"这一真实场景）

> 这是把【03-思维导图.md】那张静态大图变成一台**会转的发动机**，而且它转动的对象是一件真事：**ezagent 团队用这个 workspace 开发 ezagent 面向不同用户群体的真实产品**。一圈是——战略定方向 → 思维导图把方向拆成节点 → 每周选一两个节点开工（多半是 ezagent 某个面向用户群体的产品功能）→ 每天产 spec、做小块、配套产品/运营动作（三位一体）→ 团队自己用这个 workspace 来跑、把难受当场记成新节点 → 收"根源跟踪 + 三位一体 + 返工下降"指标 → 把学到的回填战略，再转下一圈。
>
...
## 〇、定位先收口：内部工具 = ezagent 的第一个产品（先内部、后外部，一票否决前先看这条）

把这条钉死，后面整套闭环才不失真：**本 workspace 是一个内部工具，同时它本身就是 ezagent 的第一个产品**——先在内部 dogfooding，适当时间再开放外部。

- **它要规范什么**：用 ezagent 规范 ezagent 团队自己的产品开发流程，做到两件事——①价值闭环从根源就被跟踪（战略/定位→痛点→体验→功能→开发→运营，每步挂在导图上、有人认领、有产物，谁也别脱节）；②每一个开发都有配合的产品和运营跟进（开发/产品/运营三位一体，不是开发完才补）。
- **唯一的用户场景（已收口）**：ezagent 团队自己。他们用这个 workspace 开发 ezagent 面向不同用户群体的真实产品——需求真实、高频、持续。旧体验=飞书+GitHub+群+脑子，旧体验较低，**净值为正**。
- **外部是后续阶段**：作为 ezagent 的第一个产品，先内部把价值跑通，适时开放给外部团队。本文及全套 df-prd **只按内部场景写**。凡对外动作——PMF 测分、PLG 自助路径、对外叙事、差异化 vs Linear——**一律标"待内部跑通后再议"**，下文周历和环节表已据此改写。真要开放外部时，另补 ICP 收窄 + 迁移路径 + 定价，不在本版范围。

**这条改了什么口径**：北极星不再是"自己给自己数闭环数"（活动指标伪装结果指标），而是换成有外部检验的真结果指标——见 §〇之二。

## 〇之二、ROI 这笔账先真算一次（它是 ezagent 第一个产品，ROI 从内部省下的成本算起）

它是 ezagent 的第一个产品，但第一阶段的回收是间接的：**"赚到"= 让 ezagent 自身的产品开发被根源跟踪 + 三位一体推进，省下来的返工/对齐/脱节成本 > 自研+维护成本**。这笔账此前反复被当口号、从没真算，本版先给口径（数字开工后填实）：

```
回本周期 = 自研投入 ÷ 每周净省

自研投入 = 9.5 周排期 × 团队人天折钱（含机会成本：这 9.5 周没做 ezagent 主线产品的什么）
每周净省 = 每周省下的（对齐时长 + 返工工时 + 开发/产品/运营脱节返修）× 团队时薪 − 每周维护成本（跑 BEAM 服务节点 / SPA build / 双向同步）
```

**机会成本必须显式写出来**（C 类不能省的一刀）：做这个 workspace 的 9.5 周，放弃的是直接开发 ezagent 其它产品功能的什么？为什么"先做这个能规范全队开发流程的工具"边际收益 > 直接堆功能？——理由是它一旦跑通，之后每个 ezagent 产品功能的开发都被根源跟踪 + 三位一体推进，省的是持续的返工/脱节成本，不是一次性的。这个论证不写出来，"投这里"就没被论证。承载层贯穿表已把"ezagent 其它产品线"列为隐藏关键相关方。
...
flowchart TD
    ...
    C["③每周价值闭环选题<br/>下注会：挑 1-2 个本周押注（多为 ezagent 某个产品功能）"]
    D["④每日 spec + 三位一体开发<br/>叶节点切到几天能完，每天产 spec、做一小块、产品/运营同步跟"]
    E["⑤dogfooding 体验<br/>团队自己用这个 workspace 开发 ezagent 的真实产品"]
    F["⑥运营动作（内部）<br/>每周 changelog 对内发；PMF/PLG 属对外、后续阶段不做"]
    G["⑦指标回收<br/>看：根源跟踪率/三位一体覆盖率/返工·对齐下降/cycle time"]
...
| ⑤ | dogfooding 体验 | 全队 | 自己用这个 workspace 开发 ezagent 真实产品时记录的真实痛点；用着难受的当场变成新节点 | workspace 自身（网页出口）+ 飞书 bitable（痛点登记） | 每周至少 1 条来自自用的真实痛点进树；不是"想象的需求" | Notion/Figma 极致 dogfooding https://colossus.com/article/inside-notion/ 、https://openai.com/index/figma-david-kossnick/ |
| ⑥ | 运营动作（内部版） | 运营（+产品） | 每周 changelog（哪怕小也发，对内发给自己团队即可）。**PMF 测分 / PLG 自助路径属对外、后续阶段，当前不做**——见 §〇收口 | 飞书 docx（changelog，经 external_mirror 镜到飞书群） | 每周 changelog 有≥1 条**自己团队可感知**的价值闭环 | Linear changelog https://lastrelease.io/blog/how-linear-uses-a-public-changelog （PMF/PLG 链接留待对外阶段） |
| ⑦ | 指标回收 | 产品（数）+ 研发（自动汇总） | 本周指标快照：**北极星（改成有外部检验的结果指标，见下）** + 输入指标（根源跟踪率/三位一体覆盖率/cycle time/返工·对齐下降/挂载覆盖） | 闭环看板（数据从节点状态机自动汇总）+ 飞书 bitable 趋势 | 每个输入指标都有数、趋势可见；指标没动→当周复盘→改下周选题 | 北极星输入指标 https://amplitude.com/books/north-star/about-north-star-framework ；`03-思维导图.md:188-209` |

> **北极星口径修正（Item 9）**：内部场景下"周闭环数"是**活动指标伪装成结果指标**（自己给自己数闭环，没有外部检验）。本版把北极星改成围绕"ezagent 自身产品开发被根源跟踪 + 三位一体推进"的真结果指标：**①根源价值跟踪率（开发任务能往上追到战略/痛点根节点、不脱节的比例）；②三位一体覆盖率（开发任务有配套产品/运营跟进的比例）；③返工与对齐成本实测下降多少（核心价值兑现，05 阶段 1 硬验收）**。"周闭环数"降级为输入指标之一，不再当北极星。开放外部后再加"第一个外部团队意向"这类对外结果指标。
...
> **仪式最小集收口（Item 7，避免系统2 负担过重 + 文档内部打架）**：6 个每周仪式对 3-10 人团队是过重的认知负担，且与本套材料"砍仪式"建议自相矛盾。**MVP 第一圈只强制两个仪式：日 spec（内圈）+ 周 changelog（周五）**。下面 3.2 周历里的下注会、探索访谈、PMF 测分、指标复盘等其余 5 个，**第一圈一律标为"可选/后置"**，等"返工/对齐下降"核心赌注被实测验证（05 阶段 1）后，再按团队规模逐个加回。3 人精简循环和 7-10 人完整循环承受力不同，加回节奏分档（见 04 团队画像分档）。
...
| **周四** | dogfooding 集中体验（环节⑤） | 全队 | 集中用 workspace 开发 ezagent 真实产品，把"用着难受"当场记成新节点 | 飞书 bitable 自用痛点登记 | ≥1 条自用痛点进树 | 可选/后置 |
...
| **周六** | PMF 测分 + 运营调整（环节⑥） | 运营 + 产品 | **（对外动作、后续阶段，当前不做）** 原为测 PMF 分定运营方向 | — | — | 不做（对外） |
| **周日** | 指标回收 + 复盘（环节⑦） | 产品 | 看看板：北极星（根源跟踪率 + 三位一体覆盖率 + 返工·对齐降幅）+ 输入指标动没动；指标没动→定位卡点→修正下周选题 | 闭环看板快照 + 飞书复盘短记 | 全部输入指标有数、趋势可见、下周选题已调 | 可选/后置 |
...
> **参照系修正（Item 5）**：下面 6 家公司**既是方法来源、也是直接竞品**——Linear 的 cycle+changelog+triage、Notion 的 dogfooding 中枢、Figma 的人人认领，都是它们产品里**已经规模化交付的能力**，不是"无人做过的方法"。此前把它们当"给自己背书的前辈"是偷换参照系；本版换回竞争参照系，每条加一栏诚实交代"它已做到什么程度、我们凭什么不同、为什么我们先内部跑通再谈开放外部"。
>
> **一句话总结论**：作为 ezagent 的第一个产品先在内部跑，我们的真正差异化不是"任务管理体验打赢它们"，而是"长在 ezagent 自己的运行时上、能把全队产品开发的根源价值跟踪 + 三位一体串起来、还顺手 dogfood 主线能力"。开放外部、正面比拼任务工具体验是后续阶段的事（见 §〇收口）。
...
- **它已做到 / 我们凭什么不同**：cycle+changelog+triage 已是 Linear 规模化交付的成熟能力、体验极好且已有 AI。我们不在这条上正面赢它；差异化是"goalie 由每人一个长期 agent 自动当 + 节点能往上追到战略根节点（根源跟踪）"，但 agent 这条恰是最高不确定性赌注（05 阶段 1 才验）。**先内部跑通对自己团队的价值，开放外部、比体验是后续阶段。**
...
- **我们借鉴哪条设计**：指标没动就改选题（⑦回⑦虚线）。**注意：周六 PMF 测分本身属对外动作、后续阶段，当前不做（见 §〇）**——这里只借"用指标驱动优先级"的思路。
- **它已做到 / 我们凭什么不同**：Superhuman 的 PMF 引擎是面向真实外部付费用户的。我们当前先在内部跑、无外部用户，套 PMF 周分是"自己给自己测"，没有外部检验意义；所以本版把它降级，北极星改成"根源跟踪率 + 三位一体覆盖率 + 返工/对齐下降"。开放外部后再启用 PMF。
...
- **它已做到 / 我们凭什么不同**：Notion 的 dogfooding 中枢已是成熟商业产品、体验远好于一个要自己跑 BEAM+SPA build 的自研工具。我们不在"中枢体验"上赢它；我们的差异是中枢里的每个产物都挂在能往上追到战略的导图节点上（根源跟踪）、且开发/产品/运营同节点协作（三位一体）。先内部把这套价值跑实，再谈开放外部，§〇已点破。
...
（**MVP 第一圈的最小闭环**）战略用 PR/FAQ 说清给"ezagent 团队自己开发产品"解决什么、定北极星=根源跟踪率 + 三位一体覆盖率 + 返工/对齐降幅 → 思维导图（机会-方案树）把它拆成有挂钩牌、能往上追到战略根节点的叶节点 → 每天产 spec、做一小块、产品/运营同步跟、产物挂回节点（日 spec 是强制最小集）→ 全队用这个 workspace 开发 ezagent 面向不同用户群体的真实产品、把难受当场变新节点 → 每周发一次对内 changelog（强制最小集）→ 周日收指标看根源跟踪/三位一体/返工降没降 → 每 4-6 周回看北极星、重塑机会树回填战略，转下一圈。下注会/访谈/PMF/复盘等其余仪式第一圈后置，待核心赌注（返工/对齐下降）经 05 阶段 1 实测验证再加回。承载这一切的是 ezagent 三件套：每人一个 agent 盯节点、external_mirror 把产物挂回节点、socialware 一个网页出口让全队对齐。**全程定位=内部工具 = ezagent 的第一个产品，先内部跑通、适时开放外部（§〇）。**
